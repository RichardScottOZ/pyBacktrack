...
for dst_trench_feature in dst_trench_features:
    ...
    for src_trench_feature in src_trench_features:
        ...
        # Find the average distance of each uniform point in the dst trench (from the src trench).
        src_to_dst_distance = 0.0
        for index in range(len(dst_trench_points)):
            src_to_dst_distance += pygplates.GeometryOnSphere.distance(src_trench_line, dst_trench_points[index])
